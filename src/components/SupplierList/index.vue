...
export default {
  ...
  watch: {
    open(newVal) {
      ...
      if (newVal) {
        this.search.searchCondition = null;
        this.bind.searchCondition = null;
        this.supplier.radioValue = null;
        this.selectRow = this.supplierOptions;
  ...
  methods: {
    ...
    submit() {
      if (this.selectType == 1) {
        ...
        if (this.selectRow.length > 0) {
          this.$emit('getSupplier', this.selectRow);
          this.supplier.open = false;
        }
      }
    },
