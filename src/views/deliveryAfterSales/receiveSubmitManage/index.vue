    ...
    >
      ...
      >
        <el-button
          ...
          v-hasPermi="['arrivalAndAftersales:receiveSubmit:receive']"
        >收货确认</el-button>
        <el-button
          size="small"
          type="text"
          @click="goReceiveDetails(data)"
          v-hasPermi="['arrivalAndAftersales:receiveSubmit:detail']"
        >查看详情</el-button>
        <span v-if="data.abnormal">
          <el-button
            ...
            v-hasPermi="['arrivalAndAftersales:receiveSubmit:launch']"
          ...
          <el-button
            ...
            v-hasPermi="['arrivalAndAftersales:receiveSubmit:relieve']"
          >{{data.abnormal.paraName}}</el-button>
        </span>
      </template>
    </ty-business-crud>
...
export default {
  ...
  data() {
    return {
      ...
      searchDateRang2: "",
      getListInterface: {
        url: "/system/delivery/group/chargeList",
        methodType: "get",
        params: {
          pageSize: 10,
          pageNum: 1,
  ...
  activated() {
    this.$refs.receiveCrud._getListData()
  },
  methods: {
    async init() {
    ...
    //重置
    resetInfo() {
      this.$refs.receiveCrud.setCheckList([])
      this.$refs.receiveCrud._getListData()
    },
    /** 收货确认 */
    async goReceive(row) {
    ...
    getListResFormatFun(res) {
      res.data.records.map(item => {
        if (item.typeId == 10) {
          ...
          item.totalChargeNum = this.getDecimal2(item.totalChargeNum, this.$store.getters
            .permission_decimal.cy_quantity_decimal)
        }
        else {
