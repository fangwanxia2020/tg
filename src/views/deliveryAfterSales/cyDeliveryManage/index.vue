...
export default {
  ...
  data() {
    return {
      ...
      throwType2: 21,
      getListInterface: {
        url: "/system/delivery/supplier/goods/deliveryList",
        methodType: "get",
        params: {
          pageSize: 10,
          pageNum: 1,
  ...
  methods: {
    async init() {
      // this.getListInterface.url = '/system/delivery/supplier/goods/deliveryList';
    ...
    getListResFormatFun(res) {
      res.data.records.map(item => {
        item.purchaseWaitTotalDeliveryNum = this.getDecimal2(item.purchaseWaitTotalDeliveryNum, this
          .$store.getters.permission_decimal.cy_quantity_decimal)
