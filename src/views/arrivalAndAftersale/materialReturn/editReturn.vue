...
export default {
  ...
  created() {
    let editStatic = true;
    this.jsonData = getAddJson(editStatic)
    this.returnedMaterialId = this.$route.query.returnedMaterialId;
    this.getReturnDetail()
  },
  computed: {
    isSelect() {
      if (this.$route.query.returnedMaterialId || this.addData.returnedMaterialTo == 1) {
        return true;
      }
      else {
        return false;
