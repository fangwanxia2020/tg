  ...
  <div class="container">
    ...
    >
      ...
      <ty-business-crud
        ...
        :columns="columns"
        :showSearchFields="['title','triggerType','publishWay']"
        ref="materialCrud"
      ...
      >
        ...
        >
          <el-link type="primary">
            <router-link
              :to="{path:'/information/insideInfoManage/add',query:{type: 3, msgId: data.msgId, name: 'inside'}}"
        ...
        >
          <el-link type="primary">
            <router-link
              :to="{path:'/information/insideInfoManage/add',query:{type:1, msgId: data.msgId, name: 'inside'}}"
            >查看人员</router-link>
          </el-link>
        </div>
        <template
        ...
        >
          <el-button
            ...
            v-hasPermi="['information:insideInfoManage:edit']"
          >编辑</el-button>
          <el-button
            type="text"
            @click="enableOrUnEnableMsg(data,0)"
            v-show="data.status==0"
            v-hasPermi="['information:insideInfoManage:enable']"
          >启用</el-button>
          <el-button
            type="text"
            @click="enableOrUnEnableMsg(data,1)"
            v-show="data.status==1"
            v-hasPermi="['information:insideInfoManage:disable']"
          >禁用</el-button>
          <el-button
            type="text"
            @click="deleteMsg(data)"
            v-hasPermi="['information:insideInfoManage:delete']"
          >删除</el-button>
        </template>
      </ty-business-crud>
    </div>
...
export default {
  ...
  methods: {
    ...
    enableOrUnEnableMsg({
      ...
      enableOrUnEnable(msgId).then(res => {
        if (res.code == '200') {
          this.$message.success(type === 0 ? '启用成功' : '禁用成功')
    ...
    handleEditFun(routePath, data) {
      ...
      this.$router.push({
        path: `/information/insideInfoManage/${routePath}`,
        query,
      });
    },
    // 多选框选中数据
