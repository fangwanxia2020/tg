...
export default {
  ...
  data() {
    return {
      ...
      fixedKeys: ["materialColorCodeName", "materialCode", "materialColorId", "fhasPurchaseNum", "goodsCode",
        ...
        'materColorStatic', 'materSizeStatic', 'dextField1'
      ],
  ...
  activated() {
    if (this.$route.query.isEdit) {
      ...
      this.$nextTick(() => {
        _purchaseId2 = this.$route.query.purchaseId
        // console.log("123789+++++++++++++++++++++++++++++", _purchaseId2)
      })
    }
  },
  beforeRouteEnter(to, from, next) {
    // console.log('to,from', to, from)
    if (to.name === 'gpEdit' || to.name === 'gpDetail') {
      next(vm => {
        vm.valType = true
      })
    }
    ...
    "$route.query": {
      handler(val, oldVal) {
        if (this.valType) {
          ...
          this.purchaseId = this.$route.query.purchaseId
          if (!_purchaseId2) {
            _purchaseId2 = this.purchaseId
          }
          else if (_purchaseId2 !== this.purchaseId || val.flag == 1) {
    ...
    async handleDetail() {
      ...
      if (res.code === 200) {
        ...
        for (let item of purchaseOrderDetail) {
          if (item.colorCodeName == '-' || item.materialColorCodeName == '-') { //  色号/颜色为"-"时
            item.materColorStatic = true;
          }
          else {
            item.materColorStatic = false;
          }
          if (item.sizeCodeName == '-' || item.materialSizeCodeName == '-') { // 门幅(尺码)"-"时
