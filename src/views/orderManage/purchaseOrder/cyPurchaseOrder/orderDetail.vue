...
export default {
  ...
  async created() {
    if (this.$route.query.isEdit == "1") {
      this.columns = [...this.columns, ...this.columns2]
    }
    else {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (to.name === 'cyEdit' || to.name === 'cyDetail') {
      next(vm => {
        vm.valType = true
      })
    }
  ...
  methods: {
    ...
    async handleDetail() {
      ...
      if (res.code === 200) {
        ...
        // console.log("123321", this.selectOrderList, this.addData)
        this.calTotal();
      }
    },
    async getCgCode() {
    ...
    //打开销售订单
    openSellOrder() {
      if (this.addData.orderType == null && this.addData.orderType != 0 || this.addData.orderType == "" &&
        this.addData.orderType != 0 || this.addData.orderType == undefined && this.addData.orderType != 0) {
