      ...
      <div class="content">
        ...
        <div class="btn-list">
          <el-button
            ...
            @click="allIssueFun"
            :disabled='addFlag'
            v-hasPermi="['antenatal:prenatalSample:addDelivery','antenatal:finishedSample:addDelivery']"
          >添加发样信息</el-button>
          <!-- <el-checkbox v-model="checked" @change="changeShow"
            >只显示未下发订单</el-checkbox
          > -->
        </div>
        <div class="table-box">
          <ty-table
...
export default {
  ...
  created() {
    this.getUrlStr();
    this.getList();
    this.getSampleDeliverLastdate()
  },
  watch: {},
  computed: {
    addFlag() {
      let time = new Date().getTime()
      return time > this.SampleDeliver
    }
  },
  methods: {
    getSampleDeliverLastdate() {
      getInfo({
        sealingId: this.sealingId
      }).then(res => {
        if (res.code == 200) {
          // this.SampleDeliver = res.data.sampleDeliverLastdate
          let date = new Date(res.data.sampleDeliverLastdate)
          this.SampleDeliver = date.getTime()
          // console.log("this.SampleDeliver", this.SampleDeliver)
        }
      })
    },
    //重置
    ...
    async getUrlStr() {
      ...
      this.columns = await shipmentTableColumns();
    },
    //获取typeid
    getTypeId(val) {
      this.typeId = val;
