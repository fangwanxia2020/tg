...
export default {
  ...
  data() {
    return {
      ...
      saleKey: ['originalNo', 'originalName', 'unit', 'orderNum', 'sizeCodeName', 'sizeId', 'colorCodeName',
        'colorId', 'goodsId'
      ],
  ...
  methods: {
    ...
    getSalesOrder() {
      ...
      salesOrderList(data).then(res => {
        if (res.code == 200) {
          this.tableDatas = res.data.records.filter(item => item.stNum != 0) //过滤数量为0的数据
          this.tableDatas.map((item, index) => {
            if (this.typeId == 10) {
              item.orderNum = this.getDecimal2(item.orderNum, this.$store.getters.permission_decimal
                .cy_quantity_decimal);
            }
            else {
              item.orderNum = this.getDecimal2(item.orderNum, this.$store.getters.permission_decimal
          ...
          this.page.total = this.tableDatas.length;
          this.tableData = this.tableDatas.slice((this.page.pageNo - 1) * this.page.limit, this.page
            .pageNo * this.page.limit)
        }
      })
    },
    //获取采购订单列表
      ...
      purchaseOrderList(data).then(res => {
        if (res.code == 200) {
          ...
          this.tableDatas.map((item, index) => {
            ...
            // item.stNum = this.getDecimal2(item.stNum, this.priceDic)
            // this.$set(item, 'stNum', item.stNum)
            this.$set(item, 'idx', index)
          })
          this.tableData = this.tableDatas.slice((this.page.pageNo - 1) * this.page.limit, this.page
            .pageNo * this.page.limit)
    ...
    pageSizeChange(val) {
      this.page.limit = val;
      this.tableData = this.tableDatas.slice((this.page.pageNo - 1) * this.page.limit, this.page.pageNo * this
        .page.limit)
    },
